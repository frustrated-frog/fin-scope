...
- 自动策略学院：从已冻结的公开研究目录中挑选可适配的经典策略，基于通过质量门禁的真实 A 股研究数据自动生成受限版本并发起历史实验。页面按“公开研究复现—本地历史验证—前向观察”展示证据阶梯，以确定性评分将结果放入应用候选、当前观察或学习案例；弱结果与失败不会消失，而会保留其局限和失效原因供学习。历史证据不等同于实盘证明，也不构成交易建议。
