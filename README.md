...
- 研究雷达把跨来源快讯聚合成“正在发生的事”，按新意、自选直接相关性、独立来源、来源质量和时效五项固定规则排序，并展示判断依据、信息缺口和下一观察点。实时新闻、分类结果、雷达事件、证据、解读和临时关注均只保存在 Redis，按发布时间或首次发现时间执行绝对 36 小时过期。
- 研究雷达是个人使用的短期发现层，不是公共热榜：没有运营策略后台、Prompt 配置或模型选择。需要长期保存的新闻或雷达事件必须由用户点击“记入大事记”，服务端从当前缓存读取原始内容并把不可变快照写入 SQLite；缓存已过期时不会接受客户端正文兜底入库。
- 投资观察是与首页热点、News Wire 解耦的个人研究池：它只从用户已记入大事记的新闻或雷达事件中挑选可形成投资验证问题的变化，避免临时资讯绕过归档边界写入 SQLite。它按变化强度、可信度、独立来源、持续性、影响机制和可验证性排序，并明确展示“为什么值得看、现在不能确定什么、下一验证点”；评分不代表上涨概率或交易建议。
...
- Market Pulse：按交易日冻结市场阶段、五大指数、A 股市场宽度和行业轮动。页面默认生成一份证据驱动的“今日复盘”，归纳指数全景、市场内部结构、主线与退潮、事件确认、风险和下一交易日验证清单；“市场结构”保留全部行情明细，“历史演变”对比最近 20 个已冻结交易日。行业 1/5/20 日表现来自同花顺全行业历史，不必等待本地自然积累；部分来源失败时只使用同业务日可验证数据并降低质量等级。该页面用于提高研究优先级，不直接给出买卖指令。
...
- 真实持仓策略实验室：用不可变流水手工记录 A 股期初仓位、买卖、分红、送转和资金变化，以不复权行情重建现金、移动加权成本和真实盈亏；自动复用最近一次单股预测生成独立仓位建议，并在到期后与“同一只股票保持当时持仓不动”比较。真实成本与浮亏不进入预测模型，股票发现中的“真实持有”仅作标记，不改变纯量化排名。使用方式与边界见 [真实持仓策略实验室使用说明](docs/真实持仓策略实验室使用说明.md)。
- 股票学习卡：用户只选择一只 A 股，独立的学习卡 Agent 会复用公开搜索、正文读取和 LLM 基础能力，按刘杰的空间、盈利模式、竞争、治理、定价和反方验证框架生成学习材料。六个一级维度保持固定，但各自使用不同的核心栏目和可选栏目；统一的是栏目级证据引用、验证状态与置信度。反方验证会在前五维之后综合寻找证伪条件。页面不需要研究主题或命题，单个维度失败也不会丢失其他卡片；它不写入持仓或策略命题，也不输出交易建议。
- Quant：管理研究数据集、行情同步、数据质量、因子分析、策略草稿和实验结果；资金信号可以冻结为可复现的因子研究输入。
- 自动策略学院：从已冻结的公开研究目录中挑选可适配的经典策略，基于通过质量门禁的真实 A 股研究数据自动生成受限版本并发起历史实验。页面按“公开研究复现—本地历史验证—前向观察”展示证据阶梯，并将结果分入应用候选、验证队列、当前观察或学习案例；应用候选除综合分外还要求权益曲线实际持续至少 3 年、20 笔成交且无实验警告。弱结果与失败不会消失，而会保留其年度表现、局限和失效原因供学习。历史证据不等同于实盘证明，也不构成交易建议。
- 股票发现：交易日 15:30 读取同花顺行业唯一热榜，通过扶摇同花顺 API 获取板块成分，并只允许使用有效期内的最近成功完整快照兜底；按一手成本与数据质量准入。预算只负责排除买不起一手的股票，不参与优势评分。Python 会量化全部合格候选、深度验证前 15 名，只有当前方向为上涨、成本后跑赢同股买入持有且通过样本外与模型健康门禁时才可能进入最终最多 5 只的列表。批次、来源、漏斗、概率质量、样本外证据与风险边界都会冻结到 SQLite，启动时会自动补跑最近一个已收盘工作日，页面无需手动刷新，候选可直接带入单股完整研究。
...
- 缓存：Redis 7。实时新闻与热点雷达以 Redis 为唯一运行时数据源，统一保留 36 小时；其他研究资料仍按各自策略缓存。
- 消息队列：Kafka 3.9，用于雷达预解读与股票发现异步任务。两条链路拥有独立 topic、消费组、监听容器、重试策略和 DLT，任一侧的业务失败不会阻断另一侧。
- 行情侧车：Python 3.11+、FastAPI、AkShare、pytdx、httpx，推荐通过 Docker 运行。
- AI：OpenAI 兼容 Chat Completions 客户端、多个领域 Agent、统一运行留痕。
- 持久化：SQLite 保存文章研究资产、配置和用户明确记入的大事记快照；Markdown Vault 保存长期知识，实时新闻与雷达不持久化。
...
Docker Compose 可以一次启动 Redis、Kafka、Java 后端、React/Nginx 前端和 Python 行情侧车。SQLite 与 Markdown 数据通过 `./data` 挂载到后端容器，Redis 和 Kafka 使用独立卷；Redis 重启或暂时不可用时，实时新闻与雷达返回空态或错误，不会回退到 SQLite 写入临时内容。
...
Spring Boot :8080
    ...
    ├── Redis :6379 (研究资料加速缓存)
    ├── Kafka :19092 (雷达预解读与股票发现异步任务)
    ├── RSS / Web / X / search / model providers
    └── Python market-data :8000
            ├── Fuyao Tonghuashun API (日 K / 财报 / 行业成分)
            ├── Tencent / Sina / Eastmoney / AkShare / pytdx (未覆盖能力)
...
启动后访问 `http://localhost:5173`。后端、Redis、Kafka 和行情侧车分别使用 `http://localhost:8080`、`127.0.0.1:6379`、`127.0.0.1:9092` 和 `http://localhost:8000/ready`。雷达每次完成 Top 20 快照后，会向 `finscope.radar.interpretation.requested` 发送批次消息；股票发现使用 `finscope.quant.stock-discovery.requested`。两者分别在失败重试耗尽后进入各自的 `.dlt` topic，不会互相拖垮；页面也会将“消息已送达”与“业务计算失败”分开展示。Kafka 或模型暂时不可用不会阻塞新榜单展示。
...
- 升级到资讯缓存架构后，启动器会以一次性迁移清除旧新闻分类、雷达事件及其临时派生记录，但保留新闻分类目录和大事记快照。删除会释放 SQLite 页面供后续复用；如需立即缩小 `finance.db` 文件，请在停服并备份后执行 `sqlite3 data/finance.db 'VACUUM;'`。
