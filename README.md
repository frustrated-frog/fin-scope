...
- Market Pulse：按交易日冻结市场阶段、五大指数和 A 股市场宽度，展示上涨/下跌/平盘家数、成交额、涨跌停、涨跌幅中位数与行业上涨比例，用于识别指数与多数股票是否共振。外部数据统一由 Python 行情侧车采集，部分来源失败时保留可验证结果和质量警告；该页面是研究筛选入口，不直接给出买卖指令。
