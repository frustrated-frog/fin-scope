...
- 股票发现：交易日 15:30 自动读取同花顺热门行业（接口异常时切换东方财富），聚合板块成分并按一手成本与数据质量准入；预算只负责排除买不起一手的股票，不参与优势评分。Python 会量化全部合格候选、深度验证前 15 名，最终最多展示 5 只，门禁不通过时允许空缺。批次、来源、漏斗、概率质量、样本外证据与风险边界都会冻结到 SQLite，启动时会自动补跑最近一个已收盘工作日，页面无需手动刷新。
