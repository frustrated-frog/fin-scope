...
- 投资观察是与首页热点、News Wire 解耦的个人研究池：它从已沉淀的雷达事件中自动挑选可形成投资验证问题的变化，按变化强度、可信度、独立来源、持续性、影响机制和可验证性排序，并明确展示“为什么值得看、现在不能确定什么、下一验证点”。观察池不依赖持仓；当高置信对象不足时会保留少量相对优先对象并显式标记证据不足，避免长期空库，也不会把评分解释为上涨概率或交易建议。
