...
public class DatabaseInitializer implements InitializingBean {
    ...
    private void createSchema() {
        ...
        jdbcTemplate.execute("CREATE TABLE IF NOT EXISTS holding_strategy_decision ("
                + "id INTEGER PRIMARY KEY AUTOINCREMENT,"
                + "instrument_code TEXT NOT NULL,"
                + "instrument_name TEXT,"
                ...
                + "action TEXT NOT NULL CHECK(action IN ('HOLD','ALLOW_ADD','REDUCE_CONCENTRATION','EXIT_TRIGGERED','ABSTAIN')),"
                + "suggested_quantity INTEGER NOT NULL DEFAULT 0,"
                + "expected_edge_after_cost REAL NOT NULL DEFAULT 0,"
                + "p10_risk_amount REAL NOT NULL DEFAULT 0,"
                + "p90_upside_amount REAL NOT NULL DEFAULT 0,"
                + "current_market_value REAL NOT NULL DEFAULT 0,"
                + "projected_weight REAL NOT NULL DEFAULT 0,"
                ...
                + "hold_return REAL,"
                + "incremental_return REAL,"
                + "input_json TEXT NOT NULL,"
                + "output_json TEXT NOT NULL,"
                + "created_at TEXT NOT NULL,"
                + "UNIQUE(instrument_code,decision_date,policy_version),"
                + "FOREIGN KEY(forecast_run_id) REFERENCES single_stock_forecast_run(id) ON DELETE RESTRICT)");
        jdbcTemplate.execute("CREATE INDEX IF NOT EXISTS idx_holding_strategy_decision_latest "
                + "ON holding_strategy_decision(instrument_code,decision_date DESC,id DESC)");
    ...
    private void initializeQuantSchema() {
        ...
        jdbcTemplate.update("UPDATE quant_strategy_candidate_origin SET source_commit_sha=? "
                ...
                QuantStrategyCatalogRepository.LEGACY_UNKNOWN_SOURCE_COMMIT);
        ...
        ensureColumn("stock_discovery_run", "attempt_count", "INTEGER NOT NULL DEFAULT 0");
