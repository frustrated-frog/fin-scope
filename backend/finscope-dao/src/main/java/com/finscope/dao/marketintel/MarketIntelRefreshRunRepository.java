...
public class MarketIntelRefreshRunRepository {
    ...
    public Optional<MarketIntelRefreshRun> findRunById(Long id){
        return jdbc.query("SELECT r.*,s.error_type AS step_error_type,s.error_message AS step_error_message " +
                ...
                "SELECT id FROM market_intel_refresh_step WHERE run_id=r.id AND error_type IS NOT NULL " +
                "ORDER BY CASE status WHEN 'FAILED' THEN 0 ELSE 1 END,id DESC LIMIT 1) WHERE r.id=?",(rs,rowNum)->{
    ...
    public Optional<MarketIntelRefreshStep> findLatestStep(Long instrumentId, String dimension) {
        List<MarketIntelRefreshStep> values = jdbc.query(
                "SELECT s.* FROM market_intel_refresh_step s " +
                        "JOIN market_intel_refresh_run r ON r.id=s.run_id " +
                        "WHERE r.instrument_id=? AND s.dimension=? " +
                        "ORDER BY s.id DESC LIMIT 1",
                (rs, rowNum) -> {
                    MarketIntelRefreshStep step = new MarketIntelRefreshStep();
                    step.setId(rs.getLong("id"));
                    step.setRunId(rs.getLong("run_id"));
                    step.setDimension(rs.getString("dimension"));
                    step.setProviderCode(rs.getString("provider_code"));
                    step.setAttempt(rs.getInt("attempt"));
                    step.setStatus(MarketIntelRefreshStep.Status.valueOf(rs.getString("status")));
                    ...
                    step.setOutputCount(rs.getInt("output_count"));
                    step.setStartedAt(LocalDateTime.parse(rs.getString("started_at")));
                    String finished = rs.getString("finished_at");
                    step.setFinishedAt(finished == null ? null : LocalDateTime.parse(finished));
                    return step;
                }, instrumentId, dimension);
