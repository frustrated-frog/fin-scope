...
public class FactorResearchAgentService {
    ...
    public FactorResearchAgentRun createPlan(Long datasetId, FactorIdentity factor, Long draftId, String question) {
        ...
        run.setDatasetFingerprint(dataset.getFingerprint() == null ? "" : dataset.getFingerprint());
        run.setFactor(factor);
        run.setResearchDraftId(draftId);
        ...
        plan.add("检查数据集质量、指纹与因子可用性");
        plan.add("核对因子版本、公式和解释边界");
        ...
        run.setMaxToolCalls(history.isEmpty() ? 4 : 5);
        run.setMaxLlmCalls(0);
        run.setMaxRunSeconds(60);
    ...
    public FactorResearchAgentRun approveAndRun(Long id) {
        ...
        try {
            ...
            if (run.getResearchDraftId() != null) {
                enforceBudget(run, calls + 1, startedNanos);
                calls++;
            ...
            datasetEvidence.put("id", dataset.getId());
            datasetEvidence.put("status", dataset.getStatus());
            datasetEvidence.put("dataKind", dataset.getDataKind());
            datasetEvidence.put("datasetLevel", dataset.getDatasetLevel());
            ...
            finding.put("verdict", analysis.getConclusion());
            finding.put("summary", summary(analysis));
            finding.set("counterEvidence", json.valueToTree(analysis.getCaveats()));
            ...
            String findingJson = json.writeValueAsString(finding);
    ...
    public FactorResearchAgentRun get(Long id) {
        ...
        value.setTrace(traces.findBySubject(SUBJECT_TYPE, id));
        return value;
    }
    ...
    private String summary(FactorAnalysis a) {
        return "结论为" + a.getConclusion() + "；有效日度 IC " + a.getSampleCount() + " 个，方向对齐 IC " + String.format("%.3f", a.getDirectionAdjustedIcMean()) + "。";
    }
    ...
    private String safe(String value) {
        return value == null ? "UNKNOWN_FAILURE" : value.substring(0, Math.min(300, value.length()));
    }
    ...
    private void trace(FactorResearchAgentRun run, String node, String input, String output, int calls) {
        ...
        trace.setSubjectType(SUBJECT_TYPE);
        trace.setSubjectId(run.getId());
        trace.setNodeName(node);
        trace.setStatus("SUCCESS");
        trace.setInput(input);
        trace.setOutput(output);
        trace.setErrorMessage("");
        trace.setDurationMs(0);
        trace.setStepId(node);
        trace.setAttempt(1);
        trace.setActionFingerprint(node + ":" + run.getId());
        trace.setInputHash("");
        trace.setOutputHash("");
        trace.setErrorType("");
        trace.setFallbackUsed(false);
        trace.setFallbackReason("");
        trace.setTerminationReason("");
        trace.setProgressDelta(1);
        trace.setBudgetSnapshot("{\"toolCallsUsed\":" + calls + ",\"maxToolCalls\":" + run.getMaxToolCalls() + ",\"llmCallsUsed\":0}");
        trace.setMetadataJson("{\"readOnly\":true}");
        traces.record(trace);
    }

    private static final class BudgetExceededException extends RuntimeException {
        private BudgetExceededException(String reason) {
            super(reason);
        }
    }
}
