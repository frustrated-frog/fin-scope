...
public class PerformanceMetrics {
    ...
    public BacktestMetrics calculate(List<EquityPoint> curve, double riskFreeRate, double turnover) {
        ...
        double first = curve.get(0).getPortfolioNav();
        double last = curve.get(curve.size() - 1).getPortfolioNav();
        ...
        int wins = 0;
        double peak = Double.NEGATIVE_INFINITY, maxDrawdown = 0;
        for (int i = 0; i < curve.size(); i++) {
            double nav = curve.get(i).getPortfolioNav();
            peak = Math.max(peak, nav);
            double drawdown = peak <= 0 ? 0 : (peak - nav) / peak;
            curve.get(i).setDrawdown(drawdown);
            maxDrawdown = Math.max(maxDrawdown, drawdown);
            if (i > 0) {
                double previous = curve.get(i - 1).getPortfolioNav();
                double daily = previous == 0 ? 0 : nav / previous - 1d;
                returns.add(daily);
                if (daily > 0) wins++;
            }
        }
        ...
        result.setCalmarRatio(safe(maxDrawdown == 0 ? 0 : result.getAnnualizedReturn() / maxDrawdown));
        result.setWinRate(returns.isEmpty() ? 0 : (double) wins / returns.size());
        result.setTurnover(safe(turnover));
        return result;
    }
    ...
    public List<AnnualPerformance> annual(List<EquityPoint> curve) {
        ...
        while (cursor < curve.size()) {
            ...
            EquityPoint before = cursor == 0 ? curve.get(cursor) : curve.get(cursor - 1);
            EquityPoint last = curve.get(end);
            ...
            for (int i = cursor; i <= end; i++) {
                peak = Math.max(peak, curve.get(i).getPortfolioNav());
                drawdown = Math.max(drawdown, peak <= 0 ? 0 : (peak - curve.get(i).getPortfolioNav()) / peak);
            }
            value.setMaxDrawdown(drawdown);
            result.add(value);
            cursor = end + 1;
        }
        return result;
    ...
    private double ratio(double last, double first) {
        return first <= 0 ? 0 : last / first - 1d;
    }
