...
public class QuantExperimentService {
    ...
    public QuantExperiment create(Long strategyVersionId) {
        if (strategyVersionId == null) throw new BusinessException(BizErrorCode.STRATEGY_VERSION_REQUIRED);
    ...
    public QuantExperiment get(Long id) { return decorate(repository.findById(id).orElseThrow(() -> new BusinessException(BizErrorCode.QUANT_EXPERIMENT_NOT_FOUND))); }
    ...
    private BusinessException rejected(QuantExperiment experiment, RuntimeException cause) {
        ...
        return new BusinessException(BizErrorCode.EXPERIMENT_QUEUE_FULL, cause);
    }
