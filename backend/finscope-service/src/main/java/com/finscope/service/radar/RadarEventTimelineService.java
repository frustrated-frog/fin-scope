...
public class RadarEventTimelineService {
    ...
    public List<RadarEventWorkspace.TimelineEntry> timeline(RadarEvent event, List<RadarSignal> signals,
        ...
        repository.appendTimeline(event.getId(), fingerprint, "FIRST_SEEN", "事件首次进入雷达",
                event.getCanonicalTitle(), "EVENT", event.getId(), event.getFirstSeenAt());
        if (signals != null) for (RadarSignal signal : signals) {
            repository.appendTimeline(event.getId(), fingerprint, "SIGNAL", "新增来源消息",
        ...
        if (evidence != null) for (RadarEvidence item : evidence) {
            repository.appendTimeline(event.getId(), fingerprint, "EVIDENCE", "新增研究证据",
        ...
        if (interpretation != null && "SUCCESS".equals(interpretation.getStatus())) {
            repository.appendTimeline(event.getId(), fingerprint, "INTERPRETATION", "事件解读已更新",
    ...
    private String fingerprint(RadarEvent event) {
        return String.valueOf(event.getEventKey()) + ':' + event.getLastSeenAt() + ':' + event.getSignalCount() + ':' + event.getEvidenceCount();
    }
