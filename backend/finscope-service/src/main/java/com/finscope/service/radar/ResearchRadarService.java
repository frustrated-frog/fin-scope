...
public class ResearchRadarService {
    ...
    public ResearchRadarView load(String requestedCategory, boolean watchlistOnly, int requestedLimit, String requestedState) {
        ...
        if (backgroundRefresh != null) {
            ...
            if (accepted) {
                return stored.withWarnings(Collections.singletonList("已请求后台生产，当前展示最近一次热点快照"));
            }
            if (backgroundRefresh.isRunning())
                return stored.withWarnings(Collections.singletonList("雷达正在后台生产，当前展示最近一次热点快照"));
            return stored;
        }
        if (!refreshLock.tryLock())
            return fallback(category, watchlistOnly, limit, state, now, "雷达正在刷新，已展示最近一次结果");
        try {
            ...
            for (RadarClusteringService.ClusterResult cluster : clustering.cluster(active)) {
                ...
                event.setScoreExplanation(String.join("；", result.getReasons()));
                ...
                repository.replaceEventSignals(saved.getId(), cluster.getLinks());
                ...
                if (matches(category, saved) && (!watchlistOnly || saved.getWatchlistRelevance() > 0))
    ...
    public ResearchRadarView loadFollowed(int requestedLimit) {
        int limit = Math.max(1, Math.min(requestedLimit, 50));
        if (workspace == null) {
            return ResearchRadarView.empty(LocalDateTime.now(clock));
        }
        Set<Long> uniqueIds = new LinkedHashSet<>(workspace.followedEventIds(limit));
        List<RadarEvent> followed = new ArrayList<>();
        for (Long eventId : uniqueIds) {
            if (followed.size() >= limit) {
                break;
            }
            repository.findEvent(eventId).ifPresent(followed::add);
        }
        Map<Long, ResearchRadarView.EventCard> index = cardIndex(followed);
        return new ResearchRadarView(cards(followed, index), Collections.<NewsFeedItem>emptyList(), Collections.<String>emptyList(), LocalDateTime.now(clock));
    }
    ...
    public ResearchRadarView.EventDetail detail(Long id) {
        ...
        try {
            if (workspace != null) opened = workspace.open(event);
        } catch (RuntimeException ignored) {
        }
        try {
            if (timeline != null) timelineEntries = timeline.timeline(event, signals, evidence, interpretation);
        } catch (RuntimeException ignored) {
        }
        try {
    ...
    private List<ResearchRadarView.EventCard> cards(List<RadarEvent> events, Map<Long, ResearchRadarView.EventCard> index) {
        ...
        for (RadarEvent event : events) cards.add(index.get(event.getId()));
        return cards;
    }
    ...
    private boolean matches(String category, RadarEvent event) {
        return "ALL".equals(category) || category.equalsIgnoreCase(event.getCategoryCode());
    }
    ...
    private String normalizeState(String value) {
        String state = value == null ? "ALL" : value.trim().toUpperCase(Locale.ROOT);
        return Arrays.asList("ALL", "UNREAD", "FOLLOWED", "LATER", "IGNORED").contains(state) ? state : "ALL";
    }
    ...
    private RadarSignal toSignal(NewsFeedItem item) {
        ...
        signal.setItemId(item.getId());
        signal.setProviderCode(item.getProviderCode());
        signal.setSourceName(item.getSourceName());
        signal.setSourceTier(item.getSourceTier());
        signal.setCategoryCode(item.getCategoryCode());
        signal.setTitle(item.getTitle());
        signal.setContent(item.getContent());
    ...
    private String hash(String value) {
        try {
            ...
            throw new IllegalStateException("无法生成雷达内容指纹", ex);
        }
    }
}
