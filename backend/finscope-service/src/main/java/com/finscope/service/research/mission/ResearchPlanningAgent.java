...
public class ResearchPlanningAgent {
    ...
    private String systemPrompt() {
        return "你是 FinScope 的受控研究规划器。只负责把研究命题拆成有限任务图，不执行工具。"
                ...
                + "以下五个必需任务不得删除、改名或替换工具与意图；可在它们之间增加不超过三个可选任务："
                + "baseline_scan|COLLECT|source_scan|BASELINE|[]；"
                + "search_support|SEARCH|public_news_search|SUPPORT|[baseline_scan]；"
                + "search_counter|SEARCH|public_news_search|COUNTER|[baseline_scan]；"
                + "assess_evidence|ASSESS|evidence_assess|ASSESS|[search_support,search_counter]；"
                + "synthesize_report|SYNTHESIS|report_synthesis|SYNTHESIS|[assess_evidence]。"
