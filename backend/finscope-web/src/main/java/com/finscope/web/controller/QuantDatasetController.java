...
public class QuantDatasetController {
    ...
    @Resource
    private QuantDatasetService service;
     ...
     */
    @PostMapping
    public ResponseEntity<QuantDataset> create(@RequestBody CreateQuantDatasetRequest request) {
     ...
     */
    @GetMapping("/{id}")
    public QuantDataset get(@PathVariable Long id) {
     ...
     */
    @PostMapping("/{id}/bars")
    public QuantDataset importBars(@PathVariable Long id, @RequestBody List<QuantDailyBar> values) {
     ...
     * @return 导入基本面后的数据集。
     */
    @PostMapping("/{id}/fundamentals")
    public QuantDataset importFundamentals(@PathVariable Long id, @RequestBody List<QuantFundamentalSnapshot> values) {
     ...
     * @return 导入股票池后的数据集。
     */
    @PostMapping("/{id}/universe")
    public QuantDataset importUniverse(@PathVariable Long id, @RequestBody List<QuantUniverseMember> values) {
    ...
    public java.util.Map<String, Object> quality(@PathVariable Long id) {
        QuantDataset value = service.get(id);
        java.util.Map<String, Object> result = new java.util.LinkedHashMap<String, Object>();
        result.put("datasetId", value.getId());
        result.put("status", value.getStatus());
        result.put("summary", value.getQualitySummary());
        result.put("fingerprint", value.getFingerprint());
        result.put("availableFactors", service.availableFactorCodes(id));
        return result;
    }
}
