...
**Status:** 已完成（2026-07-28）。按用户要求只推送功能分支，不创建 Pull Request。
...
- [x] Write decoder tests for UTF-8 BOM, response-header GBK, HTML meta GB2312 normalization and XML encoding.
- [x] Run `cd backend && mvn -pl finscope-rpc -Dtest=ResponseTextDecoderTest test` and verify failure because the contracts do not exist.
- [x] Implement immutable request/response contracts, stable error types and the decoder.
- [x] Run the same test and verify it passes.
- [x] Commit with `feat: 建立统一采集合同与字符集解码`.
...
- [x] Write local-server tests proving 429/503 retry once, 404 no retry, response-size rejection, timeout classification and response hashing.
- [x] Run the targeted test and verify missing runtime failures.
- [x] Implement one bounded GET path with a total deadline, byte-counting read, explicit retry classification and safe headers.
- [x] Run the targeted test and all `finscope-rpc` tests.
- [x] Commit with `feat: 实现有界可靠 HTTP 采集运行时`.
...
- [x] Add tests injecting a recording runtime and proving every list/detail/RSS request passes through it.
- [x] Run the three Adapter tests and verify they fail against direct connections.
- [x] Replace direct `Jsoup.connect`/`HttpURLConnection` calls with `AcquisitionRuntime` and parse returned text.
- [x] Run Adapter tests and the full RPC module tests.
- [x] Commit with `refactor: 统一网页与 RSS 采集出口`.
...
- [x] Write temporary-directory tests for metadata persistence, content-addressed files, secret redaction and offline replay.
- [x] Verify the tests fail because snapshot storage is absent.
- [x] Add the additive schema, repository, file store and replay application flow.
- [x] Run service tests and verify existing article ingestion remains unchanged.
- [x] Commit with `feat: 保存并重放原始采集快照`.
...
- [x] Write fixture tests for JSON-LD, `__NEXT_DATA__`, JavaScript shell detection and browser-disabled classification.
- [x] Verify the targeted tests fail.
- [x] Implement the static-to-embedded-to-browser decision state machine and disabled browser Adapter.
- [x] Add the concrete browser Adapter behind `finscope.acquisition.browser.enabled`; keep browser capacity isolated.
- [x] Run RPC tests and commit with `feat: 增强动态网页采集阶梯`.
...
- [x] Add tests proving Java owns retries for Provider calls and Python provider mode performs one attempt.
- [x] Add tests proving Python reuses one `AsyncClient` and closes it during application shutdown.
- [x] Verify targeted tests fail against the current double-policy and per-request client behavior.
- [x] Migrate transports without changing Provider parsing contracts.
- [x] Run backend and Python suites.
- [x] Commit with `refactor: 收敛行情采集可靠性职责`.
...
- [x] Document the final Module Seam, configuration and replay workflow.
- [x] Run `cd backend && mvn test`.
- [x] Run `cd market-data-service && uv run pytest -q`.
- [x] Search production Source Adapter code and verify no direct `Jsoup.connect` or `HttpURLConnection` remains.
- [x] Inspect `git diff`, commit documentation, and push `codex/unified-acquisition-runtime`; do not create a PR.
