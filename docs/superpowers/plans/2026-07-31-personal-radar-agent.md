...
- [x] 写测试：排序后的两个语义指纹构成对称键，保存后可读取判定、置信度和原因。
- [x] 运行 `cd backend && mvn -pl finscope-dao -Dtest=RadarPairDecisionRepositoryTest test`，确认因表或类型不存在而失败。
- [x] 增加 `radar_pair_decision` 表、领域对象和 Repository，使用 `ON CONFLICT(pair_key)` 幂等更新。
- [x] 再次运行测试并确认通过。
...
- [x] 写测试：合法 JSON 被解析；未知字段、越界置信度、非输入事实和模型异常触发保守结果。
- [x] 运行定向测试，确认类缺失导致 RED。
- [x] 使用 `LlmChatClient` 和严格 Jackson 映射实现；限制输入正文和输出长度。
- [x] 将成功、失败和回退写入 `AgentRunRepository`，`subject_type=RADAR_CLUSTER`。
- [x] 再次运行测试并确认通过。
...
- [x] 写测试：灰区 Agent 可合并、缓存避免重复调用、主体冲突不调用 Agent、A-B/B-C 形成一个连通分量。
- [x] 运行定向测试，确认当前保守拆分和代表信号算法导致 RED。
- [x] 将规则判定与最终判定分离，灰区按语义指纹查询缓存后调用 Agent。
- [x] 构建无向邻接表并使用 BFS 生成聚类，关系记录最终判断来源。
- [x] 模型和缓存异常均回退 `AMBIGUOUS` 保守拆分。
- [x] 运行聚类测试并确认通过。
...
- [x] 写测试：多信号事件使用合法标题；空值、超长、Markdown 和模型失败使用代表标题。
- [x] 运行定向测试并确认 RED。
- [x] 实现严格 JSON 标题生成和 `agent_run` 留痕。
- [x] 聚类完成时仅对多信号事件调用标题 Agent。
- [x] 运行 Radar 服务测试并确认通过。
...
- [x] 运行 `cd backend && mvn -pl finscope-dao,finscope-service -am test`。
- [x] 检查 `git diff --check` 和 `git status --short`，确认未包含用户的资金流文件。
- [x] 提交 `feat: 增加雷达灰区事件判断Agent` 并推送当前分支。
...
- [x] 写测试：高优先级事件最多执行两个白名单工具；无股票代码不调用结构化股票资料；低优先级不自动检索。
- [x] 运行定向测试并确认 RED。
- [x] 实现严格计划输出和工具白名单，复用现有 `ResearchAgentToolRegistry` 的 Adapter。
- [x] 将 Observation 摘要、数据引用和降级原因绑定到雷达事件。
- [x] 运行服务层测试并确认通过。
...
- [x] 写后端测试：详情返回证据摘要、数据引用和按事件查询的 Agent 轨迹。
- [x] 写前端测试：默认显示结论，展开后显示节点、状态、耗时、工具和回退原因，不显示完整 Prompt。
- [x] 分别运行定向测试并确认 RED。
- [x] 扩展持久化、详情模型和 UI 折叠轨迹。
- [x] 运行后端、前端全量测试和生产构建。
- [x] 提交 `feat: 增加雷达多源证据与Agent轨迹` 并推送当前分支。
