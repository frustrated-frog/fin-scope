...
- [ ] 写一个失败测试：两个排名聚类使用同一 `eventKey` 时，流水线应合并关系并只持久化一个事件。
- [ ] 运行 `mvn -pl finscope-service -Dtest=RadarHotspotProductionPipelineTest test`，确认当前因“重复事件身份”失败。
- [ ] 在 `persist` 之前按事件键归并事件关系，并按信号身份去重；不吞掉真正的持久化异常。
- [ ] 重新运行聚焦测试并提交 `fix: 合并热点雷达重复事件身份`。
...
- [ ] 写失败测试：provider 首次异常、第二次成功时返回在线数据；全部失败且无快照时异常包含每个来源和尝试次数。
- [ ] 运行 `uv run pytest tests/test_discovery_service.py tests/test_discovery_providers.py -q` 并确认新测试失败。
- [ ] 实现每源两次有界尝试、短退避和分源错误聚合；继续严格拒绝超过四天的快照。
- [ ] 增加独立于东方财富资金流接口的行业榜备用 provider，并保持来源审计字段真实。
- [ ] 运行聚焦 Python 测试和完整 Python 测试，提交 `fix: 增强股票发现数据源恢复能力`。
...
- [ ] 写失败测试：相同业务日的 `FAILED` 批次在恢复调度时再次发布，已成功批次保持幂等。
- [ ] 运行聚焦测试，确认恢复语义尚未满足。
- [ ] 保持唯一 `runKey`，重新发布失败批次；执行仍由 repository CAS 和 attempt token 防重。
- [ ] 运行聚焦测试并提交 `fix: 恢复失败的股票发现批次`。
...
- [ ] 写失败测试：两个 Listener 必须引用不同的命名 factory，两个 recoverer 必须路由到不同 DLT。
- [ ] 运行 `mvn -pl finscope-web -am -Dtest=DomainKafkaListenerConfigTest,StockDiscoveryListenerTest,RadarInterpretationBatchListenerTest -Dsurefire.failIfNoSpecifiedTests=false test` 并确认失败。
- [ ] 使用项目字段注入规范创建两个 factory；分别配置 `DefaultErrorHandler` 和 `DeadLetterPublishingRecoverer`。
- [ ] 在 Listener 注解中显式指定 factory，配置独立 DLT 名称、重试间隔和次数。
- [ ] 运行聚焦测试并提交 `fix: 隔离股票发现与热点消息失败`。
...
- [ ] 写失败测试：失败状态展示业务错误、自动重试提示和下一次调度时间，不将其描述为 Kafka 故障。
- [ ] 运行 Java Controller 与前端聚焦测试并确认失败。
- [ ] 扩展状态 DTO 并实现分层状态卡片，保持现有视觉语言和响应式布局。
- [ ] 运行聚焦测试、前端完整测试和构建，提交 `feat: 展示股票发现链路诊断状态`。
...
- [ ] 运行 Python 完整测试：`cd market-data-service && uv run pytest -q`。
- [ ] 运行 Java 相关模块测试：`cd backend && mvn -pl finscope-web -am test`。
- [ ] 运行前端测试和构建：`cd frontend && npm test -- --run && npm run build`。
- [ ] 启动依赖后检查两个消费组 lag 与两个 DLT 路由，记录实际验证结论。
- [ ] 对照《项目开发规范与代码评审清单》检查字段注入、大括号、模块落点、幂等、重试和错误映射。
- [ ] 更新 README 的 topic 与故障语义，提交并推送最终分支。
