...
本轮走查对照 `docs/superpowers/specs/2026-07-09-摄入工作流设计.md` 和当前项目流程，重点检查 Sources、Intake、Article 三段链路。
...
让 `CandidateReviewAgent` 返回评审内容和运行状态。`IntakeService` 根据运行状态写入 `SUCCESS` 或 `FALLBACK`。
...
Intake 标签页增加“稍后看”状态筛选和候选操作按钮。用户可以把候选从“待处理”收纳到“稍后看”，并且“稍后看”状态仍可 Promote。

验收：

1. Intake 状态筛选包含“稍后看”。
2. 候选卡片包含“稍后看”按钮。
3. 点击后调用 `/api/intake/candidates/{id}/status`，请求体为 `{"humanStatus":"SAVED_FOR_LATER"}`。
