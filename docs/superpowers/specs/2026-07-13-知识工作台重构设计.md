...
- `TopicReaderView` 聚合 SQLite 详情和 Vault Markdown，但首屏展示区、上下文和正文存在重复。
...
`App.tsx` 只装配入口和全局消息，不保存所有 Knowledge 列表与详情状态。页面数据按当前区域请求，避免每次全局刷新同时加载 341 条任务。
...
核心操作“记录理解”“回答问题”“复习结论”均在 Topic 页面内打开右侧编辑区或移动端全屏底部面板，不跳回另一个一级标签页。
...
避免：交易终端霓虹、通用 AI 仪表盘渐变、大量玻璃卡片、所有元素都悬浮。
...
全部使用本地字体栈，不依赖在线字体，符合本地优先原则。

### 13.4 组件规则

- 统一 8px 圆角；编辑器和大型阅读面允许 10px，不再混用 18px/24px 卡片。
- 卡片默认无位移动画；只有可点击卡片在悬停时改变边框和背景。
