...
## 14. 2026-08-23 首期实现说明

首期纵向切片已在 `codex/market-pulse` 分支落地：

- 新增 Market Pulse 顶层决策 Tab，按“市场状态、五日节奏、行业轮动、事件与行情确认、股票研究候选”展示证据链；
- 基于现有 `QuantDailyBarSource` 计算沪深 300 点时特征，过滤业务日之后的数据，避免未来数据泄漏；
- 新增版本化市场阶段与行业轮动规则，行业宽度未接入时明确降低质量状态和评分置信度；
- 复用 Radar 事件和最新成功的 Stock Discovery 冻结结果，只输出同时通过行业阶段、样本外结论和模型健康门禁的零至五只研究候选；
- 新增 SQLite 快照、历史日期查询、幂等刷新和 REST API；
- 页面完整呈现数据边界、零候选、风险与失效条件，并明确研究候选不是买入指令。

首期没有新增独立 Python 指标服务，而是复用项目既有行情 RPC 边界，减少重复数据链路。市场宽度、行业成分股宽度、成长相对收益、事件语义映射与候选到期归因仍作为后续数据源增强；在这些指标接入并经过真实评测前，系统不会自动提高规则权重或将候选升级为交易信号。
