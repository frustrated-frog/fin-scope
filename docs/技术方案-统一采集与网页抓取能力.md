...
普通 HTTP 请求当前通过不可变 `AcquisitionRequest` 设置策略，默认值为：
...
浏览器策略通过 `finscope.acquisition.browser.*` 外部配置：

- `browser.enabled=false`
- `browser.executable=`
- `browser.timeout-ms=20000`
- `browser.max-response-bytes=8388608`
...
- Web、Web List、RSS、X 与 Java 行情请求已迁移到 `AcquisitionRuntime`；生产 Adapter 不再直接创建 `Jsoup.connect` 或 `HttpURLConnection`。
- HTTP 运行时已覆盖绝对截止时间、慢速分块截止检查、有限重试、响应上限、字符集识别、错误分类、响应哈希和观察器隔离。
- `raw_snapshot` 与 `data/raw/acquisition/` 已投入使用，`FetchReplayService` 支持按快照 ID 离线读取原始正文。
- 网页策略已支持静态 DOM、JSON-LD、Next.js 内嵌数据和可选无头 Chrome；浏览器具有独立超时、响应上限、临时 profile 和并发配额。
- 行情传输重试已关闭，可靠性只由 `MarketDataGateway`/`ProviderRequestGuard` 决策；Python `provider_mode` 只尝试首选 Provider 一次，不更新健康状态、不写快照也不读取旧快照。

当前 `FetchReplayService` 提供应用层重放 Seam，尚未暴露公共 REST 接口；需要管理界面时可在不改动快照格式的前提下增加受控入口。
