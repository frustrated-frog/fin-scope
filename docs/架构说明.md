...
浏览器能力默认关闭，普通 HTTP、RSS 和行情不依赖 Chrome。需要抓取 JavaScript 空壳页时可配置：

- `FINSCOPE_ACQUISITION_BROWSER_ENABLED=true`：启用无头浏览器末级策略。
- `FINSCOPE_ACQUISITION_BROWSER_EXECUTABLE`：可选，指定 Chrome/Chromium 可执行文件；macOS 常见安装位置可自动发现。
- `FINSCOPE_ACQUISITION_BROWSER_TIMEOUT_MS`：渲染与容量等待的时间上限，默认 20000 毫秒。
- `FINSCOPE_ACQUISITION_BROWSER_MAX_RESPONSE_BYTES`：渲染 DOM 最大字节数，默认 8388608。
- `FINSCOPE_ACQUISITION_BROWSER_MAX_CONCURRENCY`：独立浏览器进程并发上限，默认 1。
