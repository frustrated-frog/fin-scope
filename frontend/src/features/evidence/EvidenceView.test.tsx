...
test('filters evidence records by source tier in the judgement workbench', async () => {
  ...
  render(<EvidenceView evidenceItems={evidenceItems} events={[{ id: 1, canonicalTitle: '事件', themeCode: 'MARKET' }]} onOpenNews={vi.fn()} />);
...
test('presents evidence as a judgement workbench instead of a flat ledger', () => {
  ...
  }]} events={[{ id: 1, canonicalTitle: '出口管制', themeCode: 'ai_startup' }]} onOpenNews={vi.fn()} />);
...
test('groups multiple evidence items from one event into one judgement card', () => {
  ...
  ]} events={[{ id: 1, canonicalTitle: 'DeepSeek 研究进展', themeCode: 'ai_startup' }]} onOpenNews={vi.fn()} />);
