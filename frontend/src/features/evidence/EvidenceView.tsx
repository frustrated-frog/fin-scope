...
export function EvidenceView({ evidenceItems, events, onOpenNews }: { evidenceItems: EvidenceItem[]; events: EventCluster[]; onOpenNews: () => void }) {
  ...
  return <section className="evidence-workbench evidence-apple-workbench">
    ...
    {!visible.length ? <div className="evidence-empty"><strong>尚未形成可研判的证据</strong><p>先在市场资讯中补充文章或证据，再回到这里审查判断与缺口。</p></div> : <div className="evidence-workbench-grid"><main>
      <section className="evidence-section"><div className="event-detail-section-head"><strong>证据链</strong><span>事实与推断分开呈现</span></div>{grouped.map((items) => <ThesisCard key={`${items[0].eventId}-${items[0].id}`} items={items} event={eventMap.get(items[0].eventId)} onOpenNews={onOpenNews} />)}</section>
    </main><aside>
...
function ThesisCard({ items, event, onOpenNews }: { items: EvidenceItem[]; event?: EventCluster; onOpenNews: () => void }) {
  ...
  return <article className={direct ? 'thesis-card thesis-card-confirmed' : 'thesis-card thesis-card-partial'}>
    <div className="thesis-card-head"><span className={direct ? 'evidence-state confirmed' : 'evidence-state partial'}>{direct ? '已证实' : '部分支持'}</span>{event && <button className="link-button" type="button" onClick={onOpenNews}>{event.canonicalTitle}</button>}</div>
