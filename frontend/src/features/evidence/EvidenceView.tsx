...
export function EvidenceView({ evidenceItems, events, onOpenEvent }: { evidenceItems: EvidenceItem[]; events: EventCluster[]; onOpenEvent: (eventId: number) => void }) {
  ...
  return <section className="evidence-workbench">
    <header className="evidence-workbench-head">
      <div><span className="section-kicker">Evidence / Judgment</span><h3>证据工作台</h3><p className="muted">先看当前能确认什么，再回到每一条可追溯的原始材料。</p></div>
      <div className="evidence-filters">
        <select aria-label="证据事件筛选" value={eventId} onChange={(e) => setEventId(e.target.value)}><option value="ALL">所有事件</option>{events.map((item) => <option key={item.id} value={item.id}>{item.canonicalTitle}</option>)}</select>
        <select aria-label="证据来源层级" value={sourceTier} onChange={(e) => setSourceTier(e.target.value)}><option value="ALL">所有来源</option>{tiers.map((item) => <option key={item}>{item}</option>)}</select>
        <select aria-label="证据类型" value={evidenceType} onChange={(e) => setEvidenceType(e.target.value)}><option value="ALL">所有类型</option>{types.map((item) => <option key={item}>{item}</option>)}</select>
      </div>
    </header>
    <div className="evidence-health"><div><span>当前证据状态</span><strong>{visible.length ? '持续研判中' : '等待材料'}</strong><small>不把材料数量误当作结论确定性</small></div><div><span>可直接确认</span><strong>{confirmed.length}</strong><small>事实 / 时间线</small></div><div><span>部分支持</span><strong>{partial.length}</strong><small>数据或影响推断</small></div><div><span>一手来源</span><strong>{primary} / {visible.length}</strong><small>监管、官方、公司</small></div></div>
    {!visible.length ? <div className="evidence-empty"><strong>尚未形成可研判的证据</strong><p>先在事件详情中补充文章或证据，再回到这里审查判断与缺口。</p></div> : <div className="evidence-workbench-grid"><main>
      <section className="evidence-section"><div className="event-detail-section-head"><strong>当前可以形成的判断</strong><span>事实与推断分开呈现</span></div>{grouped.map((items) => <ThesisCard key={`${items[0].eventId}-${items[0].id}`} items={items} event={eventMap.get(items[0].eventId)} onOpenEvent={onOpenEvent} />)}</section>
    </main><aside>
      <section className="evidence-section"><div className="event-detail-section-head"><strong>来源构成</strong></div>{tiers.map((tier) => <div className="evidence-source-row" key={tier}><span>{tier}</span><strong>{visible.filter((item) => item.sourceTier === tier).length}</strong></div>)}</section>
