...
function ExpectationDetail({ item, onClose }: { item: GlobalExpectationItem; onClose: () => void }) {
  ...
  return <div className="expectation-dialog-backdrop" role="presentation" onMouseDown={onClose}><section className="expectation-dialog" role="dialog" aria-modal="true" aria-label={`${item.question} 的变化详情`} onMouseDown={(event) => event.stopPropagation()}><header><div><p className="eyebrow">OBSERVATION TRACE</p><h4>{item.question}</h4></div><button type="button" aria-label="关闭变化详情" onClick={onClose}>×</button></header><div className="detail-probability"><strong>{item.probability}¢</strong><span>当前 YES 概率 · 最近刷新 {item.lastRefreshAt || item.observedAt || '—'}</span></div><div className="detail-windows"><span>5m <b className={movementClass(item.change5m)}>{formatMovement(item.change5m)}</b></span><span>1h <b className={movementClass(item.change1h)}>{formatMovement(item.change1h)}</b></span><span>24h <b className={movementClass(item.change24h)}>{formatMovement(item.change24h)}</b></span></div><div className="detail-history" aria-label="内存价格轨迹">{points.length ? points.map((point) => <i key={`${point.observedAt}-${point.probability}`} title={`${point.observedAt} · ${point.probability}¢`} style={{ height: `${20 + ((point.probability - min) / Math.max(1, max - min)) * 80}%` }} />) : <p>积累中：下一次价格变化会形成第一段轨迹。</p>}</div><p className="detail-observation">{item.observation}</p></section></div>;
}
...
function formatMovement(value?: number) { return value == null ? '积累中' : `${value >= 0 ? '+' : ''}${value.toFixed(1)}pp`; }
