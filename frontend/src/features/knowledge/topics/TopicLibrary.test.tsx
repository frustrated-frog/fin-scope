...
test('searches on the server after debounce and keeps creation in a dialog', async () => {
  ...
  fireEvent.change(screen.getByRole('searchbox', { name: '搜索投资认识' }), {
    target: { value: 'agent' }
  });
  await act(async () => {
...
test('hides article-derived material without offering a material tab', async () => {
  render(
    <TopicLibrary
      topics={[
  ...
  expect(screen.queryByRole('button', { name: /待提炼材料/ })).not.toBeInTheDocument();
  expect(screen.queryByText(/文章和自动摘要/)).not.toBeInTheDocument();
});
