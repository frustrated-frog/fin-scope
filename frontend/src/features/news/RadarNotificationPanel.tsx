...
export function RadarNotificationPanel({ hint = 0, onOpenEvent }: { hint?: number; onOpenEvent: (eventId: number) => void }) {
  ...
  async function toggle(){const next=!open;setOpen(next);if(next){setLoading(true);try{setCenter(await api<RadarNotificationCenter>('/api/research-radar/notifications?limit=30'));}finally{setLoading(false);}}}
  ...
  return <div className="radar-notification-shell"><button type="button" className="ghost-button radar-notification-trigger" aria-expanded={open} onClick={()=>void toggle()}>关注提醒 <span>{center?.unreadCount??hint}</span></button>
    {open?<div className="radar-notification-panel"><header><div><strong>变化提醒</strong><small>今日 {center?.todayCount??0} 条</small></div>{center?.unreadCount?<button type="button" onClick={()=>void readAll()}>全部已读</button>:null}</header>
      {loading?<p>正在读取提醒…</p>:center?.items.length?<ul>{center.items.map((item)=><li key={item.id??item.notificationType} className={item.read||item.id===undefined?'':'is-unread'}>{item.id===undefined?<div><strong>{item.title}</strong><span>{item.message}</span></div>:<button type="button" onClick={()=>void openItem(item.id!,item.eventId)}><strong>{item.title}</strong><span>{item.message}</span></button>}</li>)}</ul>:<p>关注事件出现新变化后会显示在这里。</p>}
    </div>:null}</div>;
}
