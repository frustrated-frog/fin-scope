...
export function LongTermStrategyView({ addToast, setMessage }: { addToast: (message: string, type?: 'success' | 'error' | 'info') => void; setMessage: (message: string) => void }) {
  ...
  const [asset, setAsset] = useState({ code: '', type: 'FUND', role: 'CORE', targetWeight: '', currentWeight: '0', quantity: '', averageCost: '', note: '' });
  ...
  async function addAsset(event: FormEvent) {
    ...
    const payload = { ...asset, targetWeight: Number(asset.targetWeight), currentWeight: Number(asset.currentWeight),
      quantity: asset.type === 'STOCK' && asset.quantity !== '' ? Number(asset.quantity) : null,
      averageCost: asset.type === 'STOCK' && asset.averageCost !== '' ? Number(asset.averageCost) : null };
    try { await api('/api/strategy/holdings', { method: 'POST', body: JSON.stringify(payload) }); setAssetOpen(false); setAsset({ code: '', type: 'FUND', role: 'CORE', targetWeight: '', currentWeight: '0', quantity: '', averageCost: '', note: '' }); await load(); addToast('资产已加入组合', 'success'); }
  ...
  return <section className="strategy-workspace">
    ...
    {assetOpen && <div className="modal-overlay"><form className="modal strategy-form" onSubmit={addAsset}><div className="modal-header"><div><p className="modal-kicker">Portfolio entry</p><h4>添加组合资产</h4></div></div><label>资产类型<select aria-label="资产类型" value={asset.type} onChange={e => { const type=e.target.value; setAsset({ ...asset, type, role: type === 'FUND' ? 'CORE' : 'OBSERVE', quantity: '', averageCost: '' }); }}><option value="FUND">场外基金</option><option value="STOCK">股票</option></select></label><label>标的代码<input aria-label="标的代码" value={asset.code} onChange={e => setAsset({ ...asset, code: e.target.value })} required /></label><label>组合角色<select value={asset.role} onChange={e => setAsset({ ...asset, role: e.target.value })}>{(asset.type === 'FUND' ? ['CORE','SATELLITE','DEFENSIVE','OBSERVE'] : ['OBSERVE','SIMULATED','LIVE_VALIDATION']).map(v => <option key={v} value={v}>{roleLabels[v]}</option>)}</select></label><label>目标权重<input aria-label="目标权重" type="number" min="0" max="100" value={asset.targetWeight} onChange={e => setAsset({ ...asset, targetWeight: e.target.value })} required /></label>{asset.type === 'STOCK' && <div className="strategy-form-row"><label>持有数量<input aria-label="持有数量" type="number" min="0" step="any" value={asset.quantity} onChange={e => setAsset({ ...asset, quantity: e.target.value })} placeholder="可选" /></label><label>平均成本<input aria-label="平均成本" type="number" min="0" step="any" value={asset.averageCost} onChange={e => setAsset({ ...asset, averageCost: e.target.value })} placeholder="可选" /></label></div>}<label>配置理由<textarea value={asset.note} onChange={e => setAsset({ ...asset, note: e.target.value })} /></label><div className="modal-actions"><button type="button" className="ghost-button" onClick={() => setAssetOpen(false)}>取消</button><button type="submit">保存资产</button></div></form></div>}
