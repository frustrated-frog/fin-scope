...
export function NextSessionValidationSummary({ records, code }: { records: NextSessionPredictionRecord[]; code?: string }) {
  ...
  return <div className="next-validation" aria-label="真实预测效果验收">
    ...
    <p>全历史冻结账本共 {summary?.recordCount ?? '—'} 条。按版本分组，同股同目标日保留最早生成的一条；先按日内股票平均，再按交易日等权。下方明细只展示最近 100 条。</p>
    ...
    {summary?.groups.map(group => <article key={group.version} className="next-validation-version">
      <h5>{group.version}</h5>
