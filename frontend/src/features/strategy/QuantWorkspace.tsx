...
export function QuantWorkspace({ addToast, setMessage }: { addToast: Toast; setMessage: (message: string) => void }) {
  ...
  async function analyzeFactor(code: string) {
    ...
    try { const value = await api<QuantFactorAnalysis>(`/api/quant/factors/${code}/analysis?datasetId=${selectedDatasetId}`); setFactorAnalyses(current => ({ ...current, [code]: value })); addToast(`${code} 因子诊断完成`, 'success'); }
