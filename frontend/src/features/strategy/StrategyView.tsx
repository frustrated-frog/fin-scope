...
import { StrategyOverview, StrategyPlaybook, StrategyReview, StrategyStockThesis } from '../../shared/types';
...
export function StrategyView({ addToast, setMessage }: { addToast: (message: string, type?: 'success' | 'error' | 'info') => void; setMessage: (message: string) => void }) {
  ...
  return <section className="strategy-workspace">
    ...
    {!loading && tab === 'playbooks' && <div className="strategy-card-grid">{playbooks.map(item => <article className="strategy-playbook" key={item.code}><header><span>{item.scope}</span><em data-status={item.status}>{statusLabels[item.status]}</em></header><h4>{item.title}</h4><p>{item.summary}</p><dl><div><dt>执行节奏</dt><dd>{item.cadence}</dd></div><div><dt>风险边界</dt><dd>{item.riskBoundary}</dd></div></dl><div className="strategy-card-actions"><button type="button" onClick={() => setPlaybook(item, 'ACTIVE')}>开始使用</button><button type="button" className="ghost-button" onClick={() => setPlaybook(item, item.status === 'PAUSED' ? 'RESEARCHING' : 'PAUSED')}>{item.status === 'PAUSED' ? '恢复研究' : '暂停'}</button></div></article>)}</div>}
