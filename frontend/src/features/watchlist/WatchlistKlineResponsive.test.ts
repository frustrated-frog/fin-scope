...
test('centers the kline overlay in the desktop workspace and resets below the sidebar breakpoint', () => {
  ...
  expect(styles).toMatch(
    /@media\s*\(max-width:\s*980px\)[\s\S]*?\.watchlist-kline-backdrop\s*{[^}]*top:\s*0;[^}]*left:\s*0;/s
  );
  expect(styles).toMatch(
