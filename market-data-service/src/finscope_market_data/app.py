...
def create_app(
    ...
    async def lifespan(application: FastAPI):
        ...
        if application.state.discovery is None:
            ...
            application.state.discovery = StockDiscoveryService(
                ...
                recall_archive=DiscoveryRecallArchive(config.data_dir / "discovery-research",
                    application.state.router.snapshots.path),
                providers=[
    ...
    @application.post("/v1/quant/holding-strategies/settle")
    async def settle_holding_decision(
    ...
    @application.get("/v1/calendar/previous-session")
    def calendar_previous_session(before: date):
        previous = previous_session(before)
        if previous is None:
